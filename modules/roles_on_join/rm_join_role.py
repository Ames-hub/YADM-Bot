...
async def handle_joinrole_rm(role_id:int, guild_id:int, respond_func):
...
class command(
    ...
    name="remove",
    description="Stop giving out a specific join role."
):

    role = lightbulb.role("role", "What role should members no longer have on join?")
