...
class command(
    ...
    async def invoke(self, ctx: lightbulb.Context) -> None:
        await prechecks("spam-am-escalation", ctx, hikari.Permissions.ADMINISTRATOR)
        ...
        if not self.del_msg_thres and not self.cooldown_thres and not self.mute_thres and not self.kick_thres and not self.ban_thres:
            ...
            if self.del_msg_thres:
                success = cat_set.escalation.msg_deletion(int(self.del_msg_thres))
                if success:
