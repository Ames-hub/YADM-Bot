...
class command(
    ...
    async def invoke(self, ctx: lightbulb.Context) -> None:
        ...
        success = guild.set.images.set_ban_msg_purgetime(seconds_duration)

        if success:
            embed = hikari.Embed(
                title="Deletion Time Set",
                description=f"When a user violates image content rules, their messages younger than {self.minutes} minutes will be deleted.",
                color=0x00ff00
            )
